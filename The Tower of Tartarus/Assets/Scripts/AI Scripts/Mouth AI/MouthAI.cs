...
public class MouthAI : MonoBehaviour
{
    ...
    //States
    MouthAIState currentState;
    public MouthAIPatrolState patrolState{get; private set;}
    public MouthAIBeginState beginState{get; private set;}


    public void ChangeState(MouthAIState newState){
    ...
    void Start()
    {
        beginState = new MouthAIBeginState(this);
        patrolState = new MouthAIPatrolState(this);
    ...
    public Player GetTarget(){
        //are we close enough?
        if(Vector3.Distance(myMouth.transform.position,targetPlayer.transform.position) > sightDistance){
            return null;
        }

        //is vision blocked by a wall?
        RaycastHit2D hit = Physics2D.Linecast(myMouth.transform.position, targetPlayer.transform.position,walls);
    ...
    {
        path.Clear();
        Vector2 closestNode = GetClosestNode(myMouth.transform.position);
    ...
    public void GetRandomMoveCommand(ref List<Vector2> path){
        ...
        Vector3 randomPos = myMouth.transform.position + randomOffset;
